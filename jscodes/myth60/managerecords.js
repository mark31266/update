...
document.getElementById("submitdata").addEventListener("click", function(event) {
  ...
  db.collection("patientvalues").doc(username + " Mythic 60").collection("DATE").doc(date1).collection("SID").doc(sid1).collection("DATA")
  .get()  
  .then((snapshot) => {
...
function updatefunction() 
{
 ...
 db.collection("auditlog").doc(date2.innerText + " " + clock1.innerText).set(
  ...
  db.collection("patientvalues").doc(username + " Mythic 60").collection("DATE").doc(date1).collection("SID").doc(sid1).collection("DATA").get().then(function(querySnapshot) 
  {
...
function deletefunction() 
{
 ...
 db.collection("auditlog").doc(date2.innerText + " " + clock1.innerText).set(
  ...
  db.collection("patientvalues").doc(username + " Mythic 60").collection("DATE").doc(date1).collection("SID").doc(sid1).delete().then(() => {
    window.location.reload(); 
}).catch((error) => {
    alert("Error removing document: ", error);
});
...
function getStories() {
    //--------------ID data---------------------//
var result = [];
db.collection("patientvalues").doc(username + " Mythic 60").collection("DATE").get().then(querySnapshot => {
...
db.collection("patientvalues").doc(username + " Mythic 60").collection("DATE").doc(x).collection("SID").get().then(querySnapshot => {
  querySnapshot.forEach(doc => result.push(doc.id));
  result.forEach( function(item) { 
    const optionObj = document.createElement("option");
