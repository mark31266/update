  ...
  //-----------------First and Last Name--------------------/
    ...
    let tr_time = document.getElementById('timeinputs');
     ...
     let th_mchcnormal = document.createElement('td');
         ...
         let th_gra = document.createElement('td');
         let th_graunit = document.createElement('td');
         let th_granormal = document.createElement('td');
     //-----------\-----PLT--------------------/
 ...
 tr_time.value = doc.data().TIME;
      ...
      if (parseFloat(mona) >= parseFloat(monb))
       ...
       results1.appendChild(tr_mon);
        ...
        //-----------------gra Data--------------------/
        var graa = doc.data().GRA; 
        var grab = doc.data().GRANORMAL.substr(7,6); 
        var grac = doc.data().GRANORMAL.substr(0,5); 
        if (parseFloat(graa) > parseFloat(grab))
           {
            th_gra.innerHTML += "" + graa + "&nbsp↑" 
           }
           else if (parseFloat(graa) < parseFloat(grac))
           {
            th_gra.innerHTML += "" + graa + "&nbsp↓" 
           }
           else 
        ...
        tr_gra.appendChild(th_gra);
        results1.appendChild(tr_gra);
        th_granormal.innerHTML += doc.data().GRANORMAL;
        tr_gra.appendChild(th_granormal);
        results1.appendChild(tr_gra);
        th_graunit.innerHTML += doc.data().GRAUNIT;
        tr_gra.appendChild(th_graunit);
        results1.appendChild(tr_gra);
...
function updatefunction() 
{
 ...
 db.collection("auditlog").doc(date2.innerText + " " + clock1.innerText).set(
  ...
  db.collection("patientvalues").doc(username + " Mythic 18").collection("DATE").doc(date1).collection("SID").doc(sid1).collection("DATA").get().then(function(querySnapshot) 
  {
